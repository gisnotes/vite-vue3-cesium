...
function setViewerParams() {
  ...
  cvs.viewer.camera.flyTo({
    destination: Cesium.Cartesian3.fromDegrees(45.0, 20.0, 17850000),
  });
}
...
function addThreeDimensionalCartesianCoordinateSystem() {
  ...
  //添加原点文字
  // addCoordinateLabel(Cesium.Cartesian3.ZERO, "原点");
  const ZERO_OFFSET = -0.03 * DEFAULT_AXIS_LENGTH;
  const zeroLabel = createLabel(
    new Cesium.Cartesian3(ZERO_OFFSET, ZERO_OFFSET, ZERO_OFFSET),
    "原点"
  );
  cvs.viewer.entities.add(zeroLabel);
  //添加坐标轴x文字标签
  const xLabel = createLabel(new Cesium.Cartesian3(LabelHeight, 0.0, 0.0), "x");
  cvs.viewer.entities.add(xLabel);
  //添加坐标轴y文字标签
  const yLabel = createLabel(new Cesium.Cartesian3(0.0, LabelHeight, 0.0), "y");
  cvs.viewer.entities.add(yLabel);
  //添加坐标轴z文字标签
  const zLabel = createLabel(new Cesium.Cartesian3(0.0, 0.0, LabelHeight), "z");
  cvs.viewer.entities.add(zLabel);
  //绘制赤道线，即0度纬线
  const equator = createParallel(0, Cesium.Color.BLUE);
  cvs.viewer.entities.add(equator);
  //添加赤道线文字标签
  const equatorLabel = createLabel(
    Cesium.Cartesian3.fromDegrees(45.0, 0.0, 300000.0),
    "赤道"
  );
  cvs.viewer.entities.add(equatorLabel);
  //绘制本初子午线，即0度经线
  const primeMeridian = createMeridian(0, Cesium.Color.RED);
  cvs.viewer.entities.add(primeMeridian);
  //添加本初子午线文字标签
  const primeMeridianLabel = createLabel(
    Cesium.Cartesian3.fromDegrees(0.0, 20.0, 300000.0),
    "本初子午线"
  );
  cvs.viewer.entities.add(primeMeridianLabel);
  //添加90度经线
  const meridian90 = createMeridian(90, Cesium.Color.YELLOW);
  cvs.viewer.entities.add(meridian90);
  //添加90度经线
  const meridian90Label = createLabel(
    Cesium.Cartesian3.fromDegrees(90.0, 20.0, 300000.0),
    "90度经线"
  );
 ...
 * @param {Cesium.Color} color 材质颜色
 * @returns {Cesium.Entity} 返回实体对象
 */
